        ...
        try{
            ...
            if(req.files){
                ...
                req.files.forEach(file=> {
                        formData.append('member-tier', fs.createReadStream(file.path), { filename: file.originalname });
                    })
            }
            console.log("formDat",formData)
            let response = await Api(req["user"].clinicBaseURL, req["user"].clinicToken).post("wallet-member-tier",formData,{
                    headers: {
                        ...formData.getHeaders() 
                    }
                })
